...
- intent_sub_type: 意图子类（如 order_status、refund_only 等），需从上面列出的子类中选择
- entities: 提取的实体信息，包含：
  * keywords: 关键词列表
  * topic: 主题
  * order_no: 订单号（如有）
...
2. 再检查是否是售后操作（refund_operation）
3. 然后检查是否是数据查询（order_query/logistics_query/product_query/coupon_query/account_query）
4. 最后才是通用知识查询（knowledge_query）
...
3. refund_operation vs knowledge_query：
    - 用户说"帮我退款""我要退货""我要换货""帮我弄退款" → refund_operation（执行操作）
    - 即使用户说"不会操作""看不清""不知道怎么弄"等困难描述，只要意图是发起售后操作，仍归为 refund_operation
    - 用户说"能不能告诉我怎么退货""教我怎么退款" → refund_operation（用户需要操作指导，本质是要发起售后）
    - 用户问"退货规则是什么""超过7天还能退吗""退货运费谁出" → knowledge_query/after_sale_policy

4. order_query vs logistics_query：
...
3. **refund_operation vs knowledge_query**：
   - "帮我退款""我要退货""帮我弄退款""能不能帮我退货" → refund_operation
   - 即使用户表达困难（"不会操作""看不清"），只要是发起售后操作仍归 refund_operation
   - "告诉我怎么退货""教我怎么退款" → refund_operation（用户希望获得操作指导并执行）
   - "退货规则""超过7天还能退吗""退货运费谁出" → knowledge_query/after_sale_policy

4. **order_query vs logistics_query**：
   - "订单状态""订单详情""取消订单""多个订单都到哪了" → order_query
   - "快递到哪了""发的什么快递""运单号查询" → logistics_query

5. **product_query vs knowledge_query**：
...
- 如果是 refund_operation，请提取售后类型和原因
...
REFUND_QUERY_PROMPT = """你是{tenant_name}的AI客服助手。用户需要处理售后（退款/退货/换货/维修）。

用户问题：{message}

请回复：
1. 告知用户售后操作需要先确认订单信息
2. 引导用户提供订单号或说明具体哪个订单
3. 说明将为其查询订单状态后判断能否进行售后操作"""


# 预留确认模板：用于售后操作的二次确认流程（当前 refund_operation_node 尚未实现确认流程，后续集成时使用）
CONFIRM_TEMPLATES = {
    "refund_only": '确认为订单 {order_id}（{product_name}）申请仅退款？退款金额 ¥{amount}，预计 {n} 个工作日到账。确认请回复\u201c确认\u201d。',
    "return_refund": '确认为订单 {order_id}（{product_name}）申请退货退款？退款金额 ¥{amount}，预计 {n} 个工作日到账。退货请保持商品完好。确认请回复\u201c确认\u201d。',
    "exchange": '确认为订单 {order_id}（{product_name}）申请换货？如需指定规格，请告知。确认请回复\u201c确认\u201d。',
    "cancel_order": '确认取消订单 {order_id}？取消后 ¥{amount} 将退回原支付账户。确认请回复\u201c确认\u201d。',
}